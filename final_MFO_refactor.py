...
def cnn_from_cfg_general(
    ...
    """
    Creates an instance of the torch_model and fits the given data on it.
    This is used during the multi-fidelity selection section. Chosen values are stored in
    the configuration (cfg).
    ...
    # determine fidelity and used budget
    img_size = max(8, int(np.floor(budget))) if fidelity == "img_size" else 32
...
def calc_spearman_correlation(confs: list[Configuration], seed: int, fidelity_budgets: dict) -> float:
    ...
    for fidelity, budgets in fidelity_budgets.items():
        ...
        eval_cheaps = [cnn_from_cfg_general(conf, fidelity, seed, budgets[0]) for conf in confs]
        print(f"fidelity: {fidelity}: eval confs with {budgets[1]} budget ")
        eval_exp = [cnn_from_cfg(conf, fidelity, seed, budgets[1]) for conf in confs]
